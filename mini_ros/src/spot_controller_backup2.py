...
class SpotCommander():
    def __init__(self, Agent=False, contacts=False):
        ...
        self.sub_imu = rospy.Subscriber('imu_data', Imu, self.imu_cb, queue_size=1) #Mexi aqui
        self.sub_cnt = rospy.Subscriber('spot/contact', ContactData, self.cnt_cb, queue_size=1)
        self.time = rospy.get_time()
    ...
    def imu_cb(self, imu): # Mexi aqui
        ...
        z = imu.orientation.z
        w = imu.orientation.w
        self.roll, self.pitch = self.euler_from_quaternion(x,y,z,w)
        self.gx = np.radians(imu.angular_velocity.x)
        self.gy = np.radians(imu.angular_velocity.y)
        self.gz = np.radians(imu.angular_velocity.z)
        self.ax = imu.linear_acceleration.x
        self.ay = imu.linear_acceleration.y
        self.az = imu.linear_acceleration.z
        # print("TESTES2")
        # self.imu = [
        #         self.roll, self.pitch,
        #         self.gx,
        #         self.gy,
        #         self.gz, self.ax, self.ay, self.az - 9.81
        #     ]
        try:
            # Update imu
            self.imu = [
    ...
    def move(self):
        ...
        # Move Type
        jt_msg.points[0].time_from_start = rospy.Duration.from_sec( dt )
